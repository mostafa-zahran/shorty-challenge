...
describe RedisDataStore::ShortyRepository do

  context 'definiation' do
    it 'should be defined' do
      expect(RedisDataStore::ShortyRepository.class).to eq(Class)
    end
    it 'should include ShortRepositoryInterface' do
      expect(RedisDataStore::ShortyRepository.included_modules).to include(ShortyApp::ShortRepositoryInterface)
    end
  end

  context 'ShortyRepository behaviour' do
    let!(:repo_obj) {RedisDataStore::ShortyRepository.new}
    ...
    context '#create!' do
      it 'can add shorty object to in memory $redis' do
        repo_obj.create!(@shorty_obj)
        expect(JSON.parse($redis.get(@short_code))['url']).to eq(@url)
      end
    end
    ...
    context '#save!' do
      it 'can modify shorty object' do
        ...
        end.to change{JSON.parse($redis.get(@shorty_obj.short_code))['redirect_count']}.by(1)
      end
    end
    ...
    context '#value' do
      it 'should return other attrs but not short_code in a hash' do
        result_hash = JSON.parse(repo_obj.send(:value, @shorty_obj))
        expect(result_hash.keys - ['url', 'start_date', 'last_seen_date', 'redirect_count']).to eq([])
        expect(result_hash['url']).to eq(@shorty_obj.url)
        expect(result_hash['start_date'].to_s).to eq(@shorty_obj.start_date.to_s)
        expect(result_hash['last_seen_date'].to_s).to eq(@shorty_obj.last_seen_date.to_s)
